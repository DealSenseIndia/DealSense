...
class AmazonDiscoverySource(DiscoverySource):
    """
    Amazon India Discovery Adapter.
    ...
    def normalize_candidate(self, raw: Dict[str, Any]) -> Optional[CandidatePayload]:
        ...
        asin = (raw.get("asin") or raw.get("merchant_product_id") or "").strip().upper()
    ...
    def fetch_candidates(self) -> List[CandidatePayload]:
        ...
        logger.info(f"AmazonDiscoverySource produced {len(payloads)} CandidatePayloads from {len(self.raw_items)} inputs.")
        return payloads
